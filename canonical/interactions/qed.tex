...
In the canonical formulation, all fields are fundamentally functions of the biquaternion coordinate $(q, T_B)$ where $T_B = t + i\psi + j\chi + k\xi$ is the biquaternion time. For practical calculations in this QED section, we work in the \textbf{complex-time reduction} where directional isotropy applies ($\chi = \xi = 0$), so that $T_B \to \tau = t + i\psi$. The field $\Theta(q,T_B)$ then reduces to $\Theta(q,\tau)$, and similarly for the gauge potential $A_\mu$.
...
\begin{itemize}
    ...
    \item $\Theta \in \mathbb{C}^{4 \times 4}$ is the biquaternion field (in matrix representation)
\end{itemize}
...
\begin{itemize}
    ...
    \item Additional degrees of freedom (the canonical theory can be extended to include consciousness or dark sector fields, but those extensions are not part of this public canonical core)
