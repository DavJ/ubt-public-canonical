...
In the canonical formulation, all fields are fundamentally functions of the biquaternion coordinate $(q, T_B)$ where $T_B = t + i\psi + j\chi + k\xi$ is the biquaternion time. For practical calculations in this QCD section, we work in the \textbf{complex-time reduction} where directional isotropy applies ($\chi = \xi = 0$), so that $T_B \to \tau = t + i\psi$. The field $\Theta(q,T_B)$ then reduces to $\Theta(q,\tau)$, and similarly for the gluon fields $G^a_\mu$.
