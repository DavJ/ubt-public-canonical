...
\begin{equation}
    ...
    \label{eq:tshirt}
\end{equation}
where $\Theta(q,T_B)$ is the canonical biquaternionic field defined over biquaternion time $T_B = t + i\psi + j\chi + k\xi$, $\mathcal{T}$ is the
energy–momentum source term, and $\kappa$ is a gravitational–gauge coupling
constant proportional to $8\pi G$.

\paragraph{Complex-Time Reduction.}
In the isotropic limit where $\chi = \xi = 0$, the canonical objects reduce to their complex-time forms: $T_B \to \tau = t + i\psi$ and $\Theta(q,T_B) \to \Theta(q,\tau)$. The T-shirt equation then becomes:
\begin{equation}
    \nabla^{\dagger}\nabla\,\Theta(q,\tau) = \kappa\,\mathcal{T}(q,\tau).
    \label{eq:tshirt_complex}
\end{equation}
Much of the literature uses this complex-time form for practical calculations.
