...
The energy-momentum (stress-energy) tensor for the biquaternion field $\Theta(q,T_B)$ is:
...
\noindent where:
\begin{itemize}
    \item $\Theta(q,T_B)$ is the canonical biquaternion field with $T_B = t + i\psi + j\chi + k\xi$
...
In the complex-time reduction where $\chi = \xi = 0$, so that $T_B \to \tau = t + i\psi$ and $\Theta(q,T_B) \to \Theta(q,\tau)$, the stress-energy tensor takes the same form with the reduced field $\Theta(q,\tau)$. This reduction is useful for isotropic systems and weak-field approximations.
