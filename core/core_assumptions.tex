...
These core assumptions are formulated in the \textbf{complex-time reduction} of the canonical biquaternion framework. The canonical time is the biquaternion time $T_B = t + i\psi + j\chi + k\xi$, and the canonical field is $\Theta(q,T_B)$. In this document, we work in the isotropic limit where $\chi = \xi = 0$, so that $T_B$ reduces to the complex time $\tau = t + i\psi$, and $\Theta(q,T_B)$ reduces to $\Theta(q,\tau)$.
